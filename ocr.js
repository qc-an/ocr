var ocrDemo = {
    ...
    resetCanvas: function() {
        ...
        canvas.onmouseup = function(e) {this.conMouseUp(e, ctx, canvas)}.bin(this);
    },
    ...
    onMouseDown: function(e, ctx, canvas){
        ...
        this.fillSquare(ctx, e.clientY - canvas.offsetTop);
    },
    ...
    train: function(){
        var digitCal = document.getElementById("digit").value;
        if (!digit || this.data.indexOf(1) < 0 )
    {
            alert("please type and draw a digit value in order to train the network");
            return;
        }
         this.trainArray.push({"y0": this.data, "label": parseInt(digitVal)});
         this.trainRequestCount++;
    ...
    test: function(){
     ...
     receiveResponse: function(xmlHttp) {
         ...
         var responseJSON = JSON.parse(xmlHttp.respenseText);
